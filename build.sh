...
unset CALLER_CONTAINER_CLI CALLER_CONTAINER_CLI_WAS_SET CALLER_CONTAINER_CLI_PATH
...
if [ "$CLI_CONTAINER_CLI_SEEN" = false ] && [ "$CALLER_CONTAINER_CLI_WAS_SET" = true ]; then
  case "$CALLER_CONTAINER_CLI" in
    container|podman|docker) ;;
    *)
  ...
  CALLER_CONTAINER_CLI_PATH="$(type -P -- "$CALLER_CONTAINER_CLI" 2>/dev/null)" || {
    echo "Error: requested container runtime '$CALLER_CONTAINER_CLI' is not on PATH." >&2
    exit 64
  }
  if [ ! -f "$CALLER_CONTAINER_CLI_PATH" ] || [ ! -x "$CALLER_CONTAINER_CLI_PATH" ]; then
    echo "Error: requested container runtime '$CALLER_CONTAINER_CLI' is not on PATH." >&2
    exit 64
  fi
  unset CALLER_CONTAINER_CLI_PATH
fi
...
unset APPROVED_DOCKER_HUB_USERNAME ASSISTANT_ID EXTENDED_ATTACHMENTS_ENABLED IMAGE_NAME DEPLOYMENT_MARKER LOGIN_STATUS
SCRIPT_DIR="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)"
XTRACE_WAS_ENABLED="$ENTRY_XTRACE_WAS_ENABLED"
unset ENTRY_XTRACE_WAS_ENABLED
BUILD_CONTEXT_DIR=""
MANIFEST_TEMP=""
EXTENDED_ATTACHMENTS_ENABLED=true
...
load_docker_env() {
  ...
  while IFS= read -r line || [ -n "$line" ]; do
    ...
    case "$key" in
      NEXT_PUBLIC_ASSISTANT_ID|NEXT_PUBLIC_LANGGRAPH_URL|NEXT_PUBLIC_MARKDOWN_EXTENDED_ATTACHMENTS_ENABLED) ;;
      AZURE_SUBSCRIPTION_ID|RESOURCE_GROUP|ACR_NAME|KV_NAME|SEED|CONTAINER_APP_NAME|CONTAINER_CLI|DOCKER_HUB_USERNAME|DOCKER_HUB_PAT|XTRACE_WAS_ENABLED|XTRACE_RESTORED|ALLEXPORT_WAS_ENABLED|ALLEXPORT_RESTORED|SCRIPT_DIR|BUILD_CONTEXT_DIR|MANIFEST_TEMP|DOCKER_HUB_PAT_VALUE|EXPORTED_DOCKER_HUB_PAT_VALUE|CALLER_DOCKER_HUB_USERNAME|CALLER_CONTAINER_CLI_WAS_SET|CALLER_CONTAINER_CLI|CALLER_CONTAINER_CLI_PATH|CLI_CONTAINER_CLI|CLI_CONTAINER_CLI_SEEN|CLI_CONTAINER_CLI_PATH|ENTRY_XTRACE_WAS_ENABLED|ENV_CONFIG_OUTPUT|CONFIG_LINE_COUNT|ENV_SH_SKIP_DOCKER_SYNC|ENV_SH_SKIP_DOCKER_SYNC_WAS_SET|ENV_SH_SKIP_DOCKER_SYNC_PREVIOUS|ENV_SH_SOURCE_STATUS|APPROVED_DOCKER_HUB_USERNAME|ASSISTANT_ID|EXTENDED_ATTACHMENTS_ENABLED|IMAGE_NAME|DEPLOYMENT_MARKER|LOGIN_STATUS|PATH|IFS|CDPATH|ENV|BASH_ENV|SHELLOPTS|BASHOPTS|HOME|PWD|OLDPWD|TMPDIR|PS4|BASH_XTRACEFD|PROMPT_COMMAND|BASH_COMPAT|POSIXLY_CORRECT|GLOBIGNORE|NODE_OPTIONS|DOCKER_CONFIG|REGISTRY_AUTH_FILE|LD_*|DYLD_*)
    ...
    if [ "$key" = NEXT_PUBLIC_MARKDOWN_EXTENDED_ATTACHMENTS_ENABLED ]; then
      EXTENDED_ATTACHMENTS_ENABLED="$value"
    fi
...
case "$EXTENDED_ATTACHMENTS_ENABLED" in
  true|false) ;;
  *) fail "NEXT_PUBLIC_MARKDOWN_EXTENDED_ATTACHMENTS_ENABLED must be exactly 'true' or 'false'." ;;
esac
...
if container_cli_build \
  ...
  --build-arg "NEXT_PUBLIC_MARKDOWN_EXTENDED_ATTACHMENTS_ENABLED=$EXTENDED_ATTACHMENTS_ENABLED" \
  -t "$IMAGE_NAME" \
