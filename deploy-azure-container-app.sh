...
if ENV_CONFIG_OUTPUT=$(BASH_ENV=/dev/null ENV=/dev/null \
  LANGCHAIN_API_KEY= UPLOAD_API_KEY= PASSKEY_PROXY_SECRET= NODE_OPTIONS= \
  DOCKER_CONFIG= REGISTRY_AUTH_FILE= OAUTH_REDIRECTS_CONFIRMED= \
  CALLER_OAUTH_REDIRECTS_CONFIRMED= CLI_OAUTH_REDIRECTS_CONFIRMED= \
...
while IFS= read -r config_line || [ -n "$config_line" ]; do
  ...
  case "$CONFIG_LINE_COUNT:$config_line" in
    ...
    6:BACKEND_APP_NAME=*) BACKEND_APP_NAME="${config_line#BACKEND_APP_NAME=}" ;;
    7:UI_APP_NAME=*) UI_APP_NAME="${config_line#UI_APP_NAME=}" ;;
    8:CONTAINER_APP_NAME=*) CONTAINER_APP_NAME="${config_line#CONTAINER_APP_NAME=}" ;;
    9:NEXT_PUBLIC_ASSISTANT_ID=*) NEXT_PUBLIC_ASSISTANT_ID="${config_line#NEXT_PUBLIC_ASSISTANT_ID=}" ;;
    *)
...
[ "$CONFIG_LINE_COUNT" -eq 9 ] || {
  echo "Error: env.sh returned incomplete configuration." >&2
  exit 1
}
export SEED AZURE_SUBSCRIPTION_ID RESOURCE_GROUP KV_NAME ENV_NAME \
  BACKEND_APP_NAME UI_APP_NAME CONTAINER_APP_NAME NEXT_PUBLIC_ASSISTANT_ID

set +x
unset ENV_CONFIG_OUTPUT CONFIG_LINE_COUNT config_line DEPLOY_XTRACE_WAS_ENABLED
unset LANGCHAIN_API_KEY UPLOAD_API_KEY PASSKEY_PROXY_SECRET NODE_OPTIONS DOCKER_CONFIG REGISTRY_AUTH_FILE
if [ "$CLI_OAUTH_REDIRECTS_CONFIRMED_SEEN" = true ]; then
