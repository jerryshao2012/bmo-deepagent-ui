...
- Update `README.md`, `documents/authentication/passkey-authentication.md`, `documents/deployment/vercel.md`, `documents/deployment/azure-container-apps.md`.
...
- [ ] Implement approved split contract in `docs/superpowers/specs/2026-08-11-azure-container-apps-ui-deployment-design.md` with RED tests first: `build.sh` is sole build/login/push owner and writes ignored `.deployment-build.json` atomically only after successful Docker Hub push.
