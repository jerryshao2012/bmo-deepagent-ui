...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    console.log(
      `[WS Fallback] Proxying SSE stream for thread ${threadId} to backend: ${backendUrl}`
    );
    ...
    const response = await fetch(backendUrl, {
      ...
      headers: {
        ...
        "X-API-Key":
          process.env.UPLOAD_API_KEY || process.env.LANGGRAPH_API_KEY || "",
      },
    ...
    if (!response.ok) {
      ...
      console.error(
        `[WS Fallback] Backend error (status ${response.status}): ${errorText}`
      );
      return new Response(
        `event: error\ndata: ${JSON.stringify({
