...
function ChatControlsBridge({
  ...
  onStopStreamReady: (fn: () => void) => void;
}) {
  const { stopStream } = useChatContext();

  useEffect(() => {
    onStopStreamReady(stopStream);
  }, [onStopStreamReady, stopStream]);
...
function HomePageInner({
  ...
  const triggerThreadListRefresh = useCallback(() => {
    mutateThreads?.();

    // Retry a few times because thread status/title updates can lag after stream completion.
    setTimeout(() => mutateThreads?.(), 1000);
    setTimeout(() => mutateThreads?.(), 2500);
    setTimeout(() => mutateThreads?.(), 5000);
  }, [mutateThreads]);
      ...
      <div className="flex h-screen flex-col">
        ...
        >
          <div className="flex items-center gap-4">
            ...
            />
            <h1 className="text-xl font-semibold">Applied AI Deep Agent</h1>
            <HealthIndicator />
            {!sidebar && (
              <Button
                variant="ghost"
              ...
              >
                <ChatControlsBridge
                  onStopStreamReady={(fn) => {
                    stopCurrentStreamRef.current = fn;
                  }}
                />
